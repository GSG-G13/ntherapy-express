...
const therapistInfoSchema = yup.object().noUnknown().shape({
  fullName: validateOptional({ schema: yup.string().min(7).max(30).required() }),
  phoneNumber: validateOptional({ schema: yup.string().min(8).max(12).required() }),
  major: validateOptional({ schema: yup.string().min(10).max(30).required() }),
  bio: validateOptional({ schema: yup.string().max(300).required() }),
  cvLink: validateOptional({
    schema: yup.string().url().min(20).max(150)
      .required(),
  }),
  hourlyRate: validateOptional({ schema: yup.number().max(3).required() }),
});
